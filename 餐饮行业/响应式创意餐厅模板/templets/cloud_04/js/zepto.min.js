...
function(t) {
    ...
    g(document).ready(function() {
        ...
        g(document).bind("MSGestureEnd",
        function(b) {
            if (b = 1 < b.velocityX ? "Right": -1 > b.velocityX ? "Left": 1 < b.velocityY ? "Down": -1 > b.velocityY ? "Up": null) a.el.trigger("swipe"),
            a.el.trigger("swipe" + b)
        }).on("touchstart MSPointerDown pointerdown",
        function(b) {
            if (! (h = t(b, "down")) || s(b)) e = h ? b: b.touches[0],
            b.touches && 1 === b.touches.length && a.x2 && (a.x2 = void 0, a.y2 = void 0),
            k = Date.now(),
            q = k - (a.last || k),
            a.el = g("tagName" in e.target ? e.target: e.target.parentNode),
            d && clearTimeout(d),
            a.x1 = e.pageX,
            a.y1 = e.pageY,
            0 < q && 250 >= q && (a.isDoubleTap = !0),
            a.last = k,
            c = setTimeout(v, 750),
            p && h && p.addPointer(b.pointerId)
        }).on("touchmove MSPointerMove pointermove",
        function(b) {
            if (! (h = t(b, "move")) || s(b)) e = h ? b: b.touches[0],
            c && clearTimeout(c),
            c = null,
            a.x2 = e.pageX,
            a.y2 = e.pageY,
            l += Math.abs(a.x1 - a.x2),
            f += Math.abs(a.y1 - a.y2)
        }).on("touchend MSPointerUp pointerup",
        function(b) {
            if (! (h = t(b, "up")) || s(b)) c && clearTimeout(c),
            c = null,
            a.x2 && 30 < Math.abs(a.x1 - a.x2) || a.y2 && 30 < Math.abs(a.y1 - a.y2) ? n = setTimeout(function() {
                a.el.trigger("swipe");
        ...
        g(window).on("scroll.cancelTouch", r)
    });
