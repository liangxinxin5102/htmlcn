    ...
    $.fn.circliful = function(options) {
        ...
        var settings = $.extend({
            ...
            foregroundColor: "#fff",
            backgroundColor: "rgba(41, 46, 54, 0)",
            fillColor: false,
            width: 30,
            dimension: 200,
            size: 15,
					...
					if($(this).data('icon') != undefined) {
					 ...
					 if($(this).data('type') != undefined) {
						...
						if(type == 'half') {
							...
							$(this).find('.circle-text-half').css({'line-height': (dimension / 1) + 'px', 'font-size' : size + 'px' });
						} else {
				...
				if($(this).data('info') != undefined) {
					...
					if($(this).data('type') != undefined) {
						...
						if(type == 'half') {
							...
							$(this).find('.circle-info-half').css({'line-height': (dimension * 1) + 'px', });
						} else {
							$(this).append('<span class="circle-info">' + info + '</span>');
							$(this).find('.circle-info').css({'line-height': (dimension * 1) + 'px', });
						}
					} else {
						$(this).append('<span class="circle-info">' + info + '</span>');
						$(this).find('.circle-info').css({'line-height': (dimension * 1) + 'px', });
