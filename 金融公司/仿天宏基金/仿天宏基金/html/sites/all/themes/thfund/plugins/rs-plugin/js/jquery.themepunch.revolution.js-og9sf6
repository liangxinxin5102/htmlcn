	...
	jQuery.fn.extend({
		...
		revolution: function(options) {
				...
				defaults = {
					...
					hsuperllCaptionAtLimit:0,				// Hide all The Captions if Width of Browser is less then this value
		...
		*******************************************/
		...
		function initSlider(container,opt) {
						...
						if (is_mobile() && opt.hsuperrrowsOnMobile=="on")
						...
						if (!container.hasClass("revslider-initialised")) {
									...
									opt.slsupermount = container.find('>ul:first >li').length;
									...
									// CREATE BULLETS
									if (opt.slsupermount >1) createBullets(container,opt);

									if (opt.slsupermount >1 && opt.navigationType=="thumb") createThumbs(container,opt);
									if (opt.slsupermount >1) createArrows(container,opt);
									...
									// START COUNTDOWN
									if (opt.slsupermount >1) countDown(container,opt);
									setTimeout(function() {
...
********************************/
		...
		********************************/
		...
		var createBullets = function(container,opt) {
			...
			bullets.find('.bullet').each(function(i) {
				var bul = jQuery(this);
				if (i==opt.slsupermount-1) bul.addClass('last');
		...
		var createArrows = function(container,opt) {
						...
						var hidden="",
							starthsuperrrows = "hsuperrrows";

						if (opt.hideThumbs==0) starthsuperrrows = "";
						...
						function aArrow(dir) {
							container.parent().append('<div style="'+hidden+'" class="tp-'+dir+'arrow '+starthsuperrrows+' tparrows '+arst+'"><div class="tp-arr-allwrapper"><div class="tp-arr-iwrapper"><div class="tp-arr-imgholder"></div><div class="tp-arr-imgholder2"></div><div class="tp-arr-titleholder"></div><div class="tp-arr-subtitleholder"></div></div></div></div>');
						}
						aArrow("left");
						aArrow("right");
						...
						container.parent().find('.tp-rightarrow').click(function() {
							...
							if (opt.transition==0) {
									...
									if (opt.next >= opt.slsupermount) opt.next=0;
						...
						container.parent().find('.tp-leftarrow').click(function() {
							if (opt.transition==0) {
									...
									if (opt.next < 0) opt.next=opt.slsupermount-1;
									callingNewSlide(opt,container);
							}
						});
		...
		*******************************************************/
		...
		var handleSpecialPreviews = function(opt) {
			...
			if (opt.beforli<0) opt.beforli = opt.slsupermount-1;
			if (opt.comingli>=opt.slsupermount) opt.comingli = 0;
		...
		********************************/
		...
		var swapSlideProgress = function(container,opt) {
			...
			setTimeout(function() {
			    ...
			    //container.trigger('nulltimer');
			    container.trigger('restarttimer');
				slsupernimation(container,opt,nextli,actli,actsh,nextsh);
			},removetime)
		...
		*******************************************/
		...
		var slsupernimation = function(container,opt,nextli,actli,actsh,nextsh) {
		...
		**************************************/
		...
		var  onPlayerStateChange = function(event) {
			...
			if (event.data == YT.PlayerState.PLAYING) {
				...
				if (event.data==0 && opt.nextslsupertend==true)
				...
				var animateTheCaptions = function(nextli, opt,recalled,mtl) {
						...
						allcaptions.each(function(i) {
								...
								if (nextcaption.hasClass("tp-static-layer")) {
									...
									if ( nes== -1 || nes == "-1")
										nextcaption.data('endslide',opt.slsupermount);

									if (nss==0 && nes==opt.slsupermount-1)
										nextcaption.data('endslide',opt.slsupermount+1);
								...
								if (opt.width<opt.hideCaptionAtLimit && nextcaption.data('captionhidden')=="on") {
									...
									if (opt.width<opt.hsuperllCaptionAtLimit || opt.width<opt.hsuperllCaptionAtLilmit)	{
										nextcaption.addClass("tp-hidden-caption")
										handlecaption=1;
									} else {
								...
								if (handlecaption==0) {
									...
									if (nextcaption.hasClass("tp-videolayer") || nextcaption.find('iframe').length>0 || nextcaption.find('video').length>0 ) {
										...
										nextcaption.find('iframe').each(function() {
												...
												opt.nextslsupertend = nextcaption.data('nextslsupertend');
		...
		var countDown = function(container,opt) {
			...
			if (opt.slsupermount >1 && !(opt.stopAfterLoops==0 && opt.stopAtSlide==1) ) {
					var bt=container.find('.tp-bannertimer');
					...
					container.on('nulltimer',function() {
					 ...
					 var countDownNext = function() {
						...
						container.trigger("revolution.slide.slsupertend");
