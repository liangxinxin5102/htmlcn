var validateFunction = {
	...
	"lengthCheck": function(value, limit1, limit2) {
		var length = value.length;
		if (length >= limit1 && length <= limit2) {
			return true;
		}
		return false;
	},
	...
	"isDate": function (value) {
		var result = value.match(/^(\d{4})(-)(\d{2})\2(\d{2})$/);
        if (result == null)
            return false;
	...
	"isMobile": function(value) {
		var pattern1 = /^((13[0-9])|(14[5|7])|(15[0|1|2|3|5|6|7|8|9])|(17[6|7|8])|(18[0-9]))\d{8}$/;
		var pattern2 = /^170[0|5|9]\d{7}$/;
	...
	"greaterThenNow": function(value) {
		...
		var now = year + "-" + (month < 10?  ("0" + month) : month)
		           + "-" + (date < 10 ? ("0" + date) : date);
