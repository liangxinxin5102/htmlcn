...
/***
 * 
 * @param toubiaojine 投标金额
 * @param borrowType  标类型 1、天标 0、不是天标
 * @param lianlilv    年利率
 * @param month       期限 
 * @returns
 */
function calc2(toubiaojine, borrowType, lianlilv, month,refundWay){
	if(refundWay==1 && month==1) refundWay = 2;
	else if(refundWay==1 && month!=1) refundWay = 0;
	else refundWay = 2;
	var toubiaojine = toubiaojine,
    lianlilv = lianlilv,
    month = month,
    fee = parseFloat(0),
    toubiaojiangli = 0,
