...
/* JS代码增加_start  By , */
AddToCartEffectOn = true;
var _JSON = {};
_JSON.stringify = function(obj){
	if(Object.prototype.toString.call(obj) != "[object Object]") return "参数错误";
	var arr=[],str="";	
	for(var i in obj){
...
function reg_package(){
	if(!document.getElementById("package_tit")){ return;};
...
function AddToCartMovie(id){
	if($(".goodsItemh").length>0){
		  $(".goodsItemh").each(function(index, element) {
			  	var num = $(this).find(".gbuycart").find("a").attr("href").split("addToCart(")[1].split(")")[0];
            	if(num == id){
					_obj = $(this).find(".gbuycart").find("a");
					return false;
					}
        	});
		  }
		if($(".textInpaddbuy").length>0){
			_obj = $(".textInpaddbuy").find("a").last();
		}
		var css ="width:30px;height:27px;z-index:9000;position:absolute;left:"+_obj.offset().left+"px;top:"+_obj.offset().top+"px;";
		var div = "<div class='AddToBuyCar' style='"+ css +"'>"+ _obj.html() +"</div>"
		$("body").append(div);
		//var $obj = $(".buyt").offset();
		var $obj = $(".icon_com.icon_shop").offset();
		$(".AddToBuyCar").animate({"top":$obj.top+"px","left":$obj.left+30+"px"},500,function(){
			$(".AddToBuyCar").remove();
		});
	}
function addToCartResponse2(obj){
	/*if(obj.content !=0){
    ...
    if (cartInfo)
    {
		function getArr(){
			...
			$("#ECS_CARTINFO dl").each(function(index, element) {
				var goodid = $(this).find(".p-name a").attr("href").split("id=")[1];
				var nums = $(this).find("strong").text().slice(-1);
				arr.push([goodid,nums]);
			});
			return arr;
			}
		var arr = [],_arr=[],id;
		...
		if(_arr.length>arr.length && arr.length>0){
		...
		var css ="width:30px;height:27px;position:absolute;left:"+_obj.offset().left+"px;top:"+_obj.offset().top+"px;";
		var div = "<div class='AddToBuyCar' style='"+ css +"'>"+ _obj.html() +"</div>"
		$("body").append(div);
		//var $obj = $(".buyt").offset();
		var $obj = $(".pub-buycar").offset();
...
function newPrice(id)
{
  ...
  toAJAX('snatch.php?act=new_price_list&id=' + id, '', newPriceResponse);
}
...
function sendHashMail()
{
 ...
 toAJAX('user.php?act=send_hash_mail', '', sendHashMailResponse);
}
...
function orderQuery()
{
  ...
  toAJAX('user.php?act=order_query&order_sn=s' + order_sn, '', orderQueryResponse);
}
...
function addPackageToCart(packageId, indexId) 
{
  ...
  toAJAX('flow.php?step=add_package_to_cart', 'package_info=' + _JSON.stringify(package_info), addPackageToCartResponse);
}
/* 代码修改_end By , */
...
function submit_div(goods_id, parentId) 
{
  ...
  //Ajax.call('flow.php?step=add_to_cart', 'goods=' + goods.toJSONString(), addToCartResponse, 'POST', 'JSON');
	toAJAX('flow.php?step=add_to_cart', 'goods=' + _JSON.stringify(goods), addToCartResponse);
