var img = {
	 ...
	 init:function(){
		...
		img.autoplay= setInterval(function(){img.play(img.num+1)},2000);
		
		img.imageShow.onmouseover = function(){clearInterval(img.autoplay);}
		img.imageShow.onmouseout = function(){img.autoplay= setInterval(function(){img.play(img.num+1)},2000); }
	...
	play:function(i){
		...
		if(i>img.num){
			...
			if(parseInt(img.current.style.left)>240){
				//如果活动框到了最右边--完成
				//小图片向左
