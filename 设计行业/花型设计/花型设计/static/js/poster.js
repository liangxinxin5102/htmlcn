(function(jq){
	var posterGrid = function(o, options, data){
		this.parent = jq("#"+ o);
		this.body   = jq("body");
		...
		this.options = jq.extend({}, posterGrid.options, options);
	...
	posterGrid.prototype = {
		...
		drawContent: function(){
			...
			this.content.cssText = "width:"+this.options.width+"px;height:"+this.options.height+"px;cursor:move;position:absolute;";
				...
				this.bottomNav.className = this.options.hidenav ? "bottomNav hidden" : "bottomNav";
	...
	posterGrid.options = {
		...
		className : "posterGrid",//最外层样式
		autoWidth : true,//默认不用设置宽
		width : 970,//最外层宽，需要使用的时候在传,默认由程序自动判断
		height : 310,//最外层高  
		delay : 5000*3600,//滚动间隔（毫秒）
		speed : 500 //滚动速度毫秒
	};
