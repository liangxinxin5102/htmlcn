...
(function(window, document, undefined) {    
    ...
    function scanElementsByTagName(tagName) {
        ...
        for (n = 0; n < elementsList.length; n++) {
			...
			var createSWFplay = false;	//鏄惁鐢╯wf鎾斁
			var videoUrl = "";
            // Test if the media tag is supported.
            if (element.canPlayType) {
                // If the media has a src attribute, and can play it, then all is good.
				if (element.src) {
                    if ( canPlayFormat(element, guessFormat(tagName, element.src)) && !(element.src.search(/.flv/i) > 0) ) {    //澧炲姞鏄惁鏄痜lv鏍煎紡瑙嗛鐨勫垽鏂�
                    ...
                    for (var m = 0; m < sources.length; m++) {
                        ...
                        if ( canPlayFormat(element, guessFormat(tagName, source.src, source.type)) && !(source.src.search(/.flv/i) > 0) ) {    //澧炲姞鏄惁鏄痜lv鏍煎紡瑙嗛鐨勫垽
            ...
            // If cannot play media, create the fallback. 鏄惁浣跨敤flowplay鎾斁瑙嗛鍒ゆ柇
