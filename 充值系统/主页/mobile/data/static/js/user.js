...
function is_registered( username )
{
    ...
    var  reg="[\u4e00-\u9fa5]";
    var chinese=username.match(reg);
    if(chinese!=null){
         document.getElementById('username_notice').innerHTML = '用户名不能包含汉字';
...
function register()
{
  ...
  var username  = frm.elements['username'] ? Utils.trim(frm.elements['username'].value) : '';
  var email  = frm.elements['email'] ? frm.elements['email'].value : '';
  username = email;
  var password  = frm.elements['password'] ? Utils.trim(frm.elements['password'].value) : '';
  var confirm_password = frm.elements['confirm_password'] ? Utils.trim(frm.elements['confirm_password'].value) : '';
  var checked_agreement = frm.elements['agreement'] ? frm.elements['agreement'].checked : '';
