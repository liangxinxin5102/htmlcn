...
function addToCart_quick(goodsId, parentId)
{
	...
	if (formBuy)
	{
		...
		{
			/*lifei*/
			if(trim(formBuy.elements['sdoname'].value)==''){
				alert("充值账号不可为空");
				exit;
			}else{
...
function addToCartResponse(result)
{
  ...
  if (result.error > 0)
  {
    ...
    }else
    {
	  ...
	  if(result.goods_id == 1458){window.location.href="goods-1279.html";}
    }
  }
  else
  {
...
function affirm_pay(oid) {
	upTel();
	jQuery("#paypal_btn").val('正在使用余额付款');
	jQuery("#paypal_btn").addClass("cursor_not bg_color_FF8993");
	 Ajax.call('flow.php?step=affirm_pay&order_id='+oid, '', AffirmPayResponse, 'GET', 'JSON');
}
function AffirmPayResponse(result)
{
	...
	{
		alert(result.msg);
		jQuery("#paypal_btn").val('确认使用余额付款');
	}
	else if(result.code == 1)
	{
		alert(result.msg);
		jQuery("#paypal_btn").val('余额付款已经成功');
		window.location.href="user.php?act=order_detail&order_id="+result.oid;
	}

	jQuery("#paypal_btn").removeClass("cursor_not bg_color_FF8993");
}
