...
{
	//按钮点击相应交互
	//document.getElementById('BuyButton').style.background="#b2b2b2";
	document.getElementById('BuyButton').innerHTML = '正在提交...';
	...
	if (formBuy)
	{
		...
		if (formBuy.elements['number'])
		{
			...
			if(number == 0 || number == ''){
				...
				document.getElementById('BuyButton').innerHTML = '立即购买';
				return false;
			}
		}
	...
	goods.buy_url  = buy_url;  /*lifei*/
	goods.sdoname  = sdoname;  /*lifei*/
	goods.resdoname= resdoname;  /*lifei*/
	goods.fqname   = fqname;  /*lifei*/
	...
	goods.ext1     = ext1;  /*lifei*/
	goods.ext2     = ext2;  /*lifei*/
	goods.ext3     = ext3;  /*lifei*/
	goods.ext4     = ext4;  /*lifei*/
    ...
    if(flag){
    	...
    	Ajax.call('/mobile/flow.php?step=add_to_cart', 'goods=' + goods.toJSONString(), addToCartResponse, 'POST', 'JSON');
    }
}
...
//			{
				if(flag){
					...
					location.href = '/mobile/flow.php?step=checkout';
				}
//			}
		}
		// 没选规格，弹出属性选择框
		...
		/*else if (result.error == 100)//byxiaoyan20130805
		{
			if (confirm(result.message))
			{
				location.href = 'flow.php?step=login&id=' + result.goods_id;
			}
	...
	else
	{
		...
		if(flag){
			...
			var cart_url = '/mobile/flow.php?step=checkout';
		}
		if (cartInfo)
		{
